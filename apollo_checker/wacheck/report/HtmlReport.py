...
class HtmlReport(Report):
    ...
    def render_error(self, e):
        ...
        if e.code == GeneError.SYMBOL_MISSING:
            return "Gene <a href=\"" + self.get_wa_url(e.scaffold, e.start, e.end) + "\">" + e.display_id + "</a> is missing a 'symbol'."

        if e.code == GeneError.SYMBOL_INVALID:
            return "Gene <a href=\"" + self.get_wa_url(e.scaffold, e.start, e.end) + "\">" + e.display_id + "</a> have an invalid 'symbol': '" + e.error_desc['symbol'] + "'. Use only letters digits and -_.()/ characters."

        if e.code == GeneError.MULTIPLE_MRNAS:
            return "Gene <a href=\"" + self.get_wa_url(e.scaffold, e.start, e.end) + "\">" + e.display_id + "</a> have " + str(e.error_desc['num']) + " transcripts. Check that they are real splice variants and not erroneous duplications."

        if e.code == GeneError.CDS_IS_NULL:
            return "Gene <a href=\"" + self.get_wa_url(e.scaffold, e.start, e.end) + "\">" + e.display_id + "</a> CDS length is null (0). Check that the gene contains a start codon and that the exons are properly defined."

        if e.code == GeneError.CDS_IS_SMALL:
            return "Gene <a href=\"" + self.get_wa_url(e.scaffold, e.start, e.end) + "\">" + e.display_id + "</a> CDS length is very small (" + str(e.error_desc['len']) + "). Check that the gene contains a start codon and that the exons are properly defined."

        if e.code == GeneError.NAME_MISSING:
            return "Gene <a href=\"" + self.get_wa_url(e.scaffold, e.start, e.end) + "\">" + e.display_id + "</a> is missing a 'Name'."

        if e.code == GeneError.NAME_INVALID:
            return "Gene <a href=\"" + self.get_wa_url(e.scaffold, e.start, e.end) + "\">" + e.display_id + "</a> should have a human readable 'Name' instead of '" + e.error_desc['name'] + "'."

        if e.code == GeneError.DBXREF_UNKNOWN:
            return "Gene <a href=\"" + self.get_wa_url(e.scaffold, e.start, e.end) + "\">" + e.display_id + "</a> has an unknown dbxref type: '" + e.error_desc['dbxref'] + "'."

        if e.code == GeneError.GROUP_MISPLACED:
            return "Gene <a href=\"" + self.get_wa_url(e.scaffold, e.start, e.end) + "\">" + e.display_id + "</a> have " + e.error_desc['tag'] + " as a gene DBXref instead of an attribute."
        ...
        if e.code == GeneError.ATTRIBUTE_INVALID:
            return "Gene <a href=\"" + self.get_wa_url(e.scaffold, e.start, e.end) + "\">" + e.display_id + "</a> have an invalid '" + e.error_desc['key'] + "' attribute value ('" + e.error_desc['value'] + "')"

        if e.code == GeneError.PART_SAME:
            return "Gene <a href=\"" + self.get_wa_url(e.scaffold, e.start, e.end) + "\">" + e.display_id + "</a> and " + e.error_desc['other_name'] + " located at " + self.get_wa_url(e.error_desc['other_scaff'], e.error_desc['other_start'], e.error_desc['other_end']) + " are marked as the same part of a gene. If they are the same part of different alleles, add 'Allele' tags to both."

        if e.code == GeneError.PART_SINGLE:
            return "Gene <a href=\"" + self.get_wa_url(e.scaffold, e.start, e.end) + "\">" + e.display_id + "</a> is the only part of a gene. If the gene is complete, you should remove the 'part' tag. Otherwise, check that all the parts of the gene have exactly the same 'symbol'. If the gene is incomplete, leave it like this."

        if e.code == GeneError.PART_SINGLE_NAMED:
            return "Gene <a href=\"" + self.get_wa_url(e.scaffold, e.start, e.end) + "\">" + e.display_id + "</a> is the only part of a gene. If the gene is complete, you should remove the 'part' tag. Otherwise, check that all the parts of the gene have exactly the same 'symbol'. If the gene is incomplete, leave it like this."

        if e.code == GeneError.ALLELE_SAME:
            return "Gene <a href=\"" + self.get_wa_url(e.scaffold, e.start, e.end) + "\">" + e.display_id + "</a> and " + e.error_desc['other_name'] + " located at " + self.get_wa_url(e.error_desc['other_scaff'], e.error_desc['other_start'], e.error_desc['other_end']) + " are marked as the same allele of a duplicated gene. If they are different parts of the same allele, add 'Part' tags to both."

        if e.code == GeneError.ALLELE_SINGLE:
            return "Gene <a href=\"" + self.get_wa_url(e.scaffold, e.start, e.end) + "\">" + e.display_id + "</a> is the only allele of a gene. If the gene has no other allele, you should remove the 'allele' tag. Otherwise, check that all alleles of the same gene have the same 'symbol'"

        if e.code == GeneError.INTRON_TOO_SMALL:
            return "Gene <a href=\"" + self.get_wa_url(e.scaffold, e.start, e.end) + "\">" + e.display_id + "</a> contains very short introns. This is not supposed to happen on most genes, check that the gene structure is correct."
    ...
    def render_warning(self, w):
        ...
        if w.code == GeneError.SYMBOL_MISSING:
            return "Gene <a href=\"" + self.get_wa_url(w.scaffold, w.start, w.end) + "\">" + w.display_id + "</a> is missing a 'symbol'."

        if w.code == GeneError.SYMBOL_INVALID:
            return "Gene <a href=\"" + self.get_wa_url(w.scaffold, w.start, w.end) + "\">" + w.display_id + "</a> have an invalid 'symbol': '" + w.error_desc['symbol'] + "'. Use only letters digits and -_.()/ characters."

        if w.code == GeneError.MULTIPLE_MRNAS:
            return "Gene <a href=\"" + self.get_wa_url(w.scaffold, w.start, w.end) + "\">" + w.display_id + "</a> have " + str(w.error_desc['num']) + " transcripts. Check that they are real splice variants and not erroneous duplications."

        if w.code == GeneError.CDS_IS_NULL:
            return "Gene <a href=\"" + self.get_wa_url(w.scaffold, w.start, w.end) + "\">" + w.display_id + "</a> CDS length is null (0). Check that the gene contains a start codon and that the exons are properly defined."

        if w.code == GeneError.CDS_IS_SMALL:
            return "Gene <a href=\"" + self.get_wa_url(w.scaffold, w.start, w.end) + "\">" + w.display_id + "</a> CDS length is very small (" + str(w.error_desc['len']) + "). Check that the gene contains a start codon and that the exons are properly defined."
        ...
        if w.code == GeneError.GROUP_MULTIPLE_SAME:
            return "Gene <a href=\"" + self.get_wa_url(w.scaffold, w.start, w.end) + "\">" + w.display_id + "</a> has been added to the same annotation group '" + w.error_desc['group'] + "' multiple times."

        if w.code == GeneError.ATTRIBUTE_INVALID:
            return "Gene <a href=\"" + self.get_wa_url(w.scaffold, w.start, w.end) + "\">" + w.display_id + "</a> have an invalid '" + w.error_desc['key'] + "' attribute value ('" + w.error_desc['value'] + "')"

        if w.code == GeneError.PART_SAME:
            return "Gene <a href=\"" + self.get_wa_url(w.scaffold, w.start, w.end) + "\">" + w.display_id + "</a> and <a href=\"" + self.get_wa_url(w.error_desc['other_scaff'], w.error_desc['other_start'], w.error_desc['other_end']) + "\">" + w.error_desc['other_name'] + "</a> are marked as the same part of a gene. If they are the same part of different alleles, add 'Allele' tags to both."

        if w.code == GeneError.PART_SINGLE:
            return "Gene <a href=\"" + self.get_wa_url(w.scaffold, w.start, w.end) + "\">" + w.display_id + "</a> is the only part of a gene. If the gene is complete, you should remove the 'part' tag. Otherwise, check that all the parts of the gene have exactly the same 'symbol'. If the gene is incomplete, leave it like this."

        if w.code == GeneError.PART_SINGLE_NAMED:
            return "Gene <a href=\"" + self.get_wa_url(w.scaffold, w.start, w.end) + "\">" + w.display_id + "</a> is the only part of a gene. If the gene is complete, you should remove the 'part' tag. Otherwise, check that all the parts of the gene have exactly the same 'symbol'. If the gene is incomplete, leave it like this."

        if w.code == GeneError.ALLELE_SAME:
            return "Gene <a href=\"" + self.get_wa_url(w.scaffold, w.start, w.end) + "\">" + w.display_id + "</a> and <a href=\"" + self.get_wa_url(w.error_desc['other_scaff'], w.error_desc['other_start'], w.error_desc['other_end']) + "\">" + w.error_desc['other_name'] + "</a> are marked as the same allele of a duplicated gene. If they are different parts of the same allele, add 'Part' tags to both."

        if w.code == GeneError.ALLELE_SINGLE:
            return "Gene <a href=\"" + self.get_wa_url(w.scaffold, w.start, w.end) + "\">" + w.display_id + "</a> is the only allele of a gene. If the gene has no other allele, you should remove the 'allele' tag. Otherwise, check that all alleles of the same gene have the same 'symbol'"
        ...
        if w.code == GeneError.DELETED_WRONG_NAME:
            return "Gene <a href=\"" + self.get_wa_url(w.scaffold, w.start, w.end) + "\">" + w.display_id + "</a> is marked as deleted but has a wrong gene name '" + w.error_desc['name'] + "'."
